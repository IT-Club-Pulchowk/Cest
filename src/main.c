...
void DeserializeMuda(Compiler_Config_List *config_list, Uint8 *data, Compiler_Kind compiler) {
	...
	while (MudaParseNext(&prsr)) {
		...
		case Muda_Token_Property: {
					...
					if (StrMatch(info->Name, token->Data.Property.Key)) {
						...
						case Compiler_Config_Member_Enum: {
							...
							if (!found) {
								...
								LogWarn("Line: %u, Column: %u :: Invalid value for Property \"%s\" : %s. Acceptable values are: %s\n",
									prsr.line, prsr.column, info->Name.Data, token->Data.Property.Value->Data, accepted_values);
			...
			if (!property_found) {
				LogWarn("Line: %u, Column: %u :: Invalid Property \"%s\". Ignored.\n", prsr.line, prsr.column, token->Data.Property.Key.Data);
			}
		} break;
...
void ExecuteMudaBuild(Compiler_Config *compiler_config, Build_Config *build_config, const Compiler_Kind compiler, const char *parent) {
	...
	if (compiler_config->Kind == Compile_Project) {
		...
		case Compiler_Bit_CLANG: {
			...
			if (PLATFORM_OS_WINDOWS) {
			  // OutFormatted(target, "-Xlinker -subsystem:%s ", compiler_config->Subsystem == Subsystem_Console ? "CONSOLE" : "WINDOWS");
			  // <------ This one depends upon the linker used by Clang .. It could be link.exe (lld.exe) (from msvc) or ld.exe (from gcc) ------>
			  // So flag depends 
			  OutFormatted(target, "\"-Wl,--subsystem,%s\" ", compiler_config->Subsystem == Subsystem_Console ? "console" : "windows");
			}
		...
		case Compiler_Bit_GCC: {
			...
			if (PLATFORM_OS_WINDOWS) {
				OutFormatted(target, "-w -Wl,-subsystem,%s ", compiler_config->Subsystem == Subsystem_Console ? "console" : "windows");
			}
